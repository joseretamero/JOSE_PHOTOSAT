...
time+=Pr_Time(1,500000); // Add X sec + Y microsec    
...
time.GetTime(); // Get current monotonic time
time += Pr_Time(0,75000); // Add X sec + Y microsec
   //Program absolute timer 
   AttCtrlTimer.InformAt( time ); 
}
	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (DoAttCtrl):
				...
				//Execute Action 
				FDoAttitudeCtrl();
				//Evaluate Branch EvNewObs
				if( GReadyToObservation() )
				{
					//Execute Action 
					FToObservation();

					//Branch taken is DoAttCtrl_EvNewObs
			...
			case (TakeImage):
				...
				if( GLastImage() )
				{
					...
					//Branch taken is TakeImage_EvDoAttCtrl
					edroomCurrentTrans.localId =
